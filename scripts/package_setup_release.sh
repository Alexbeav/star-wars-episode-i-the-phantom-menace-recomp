...
#
# Writes: dist/star-wars-episode-i-the-phantom-menace-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix star-wars-episode-i-the-phantom-menace \
  --exe-name Star_Wars_Episode_I__The_Phantom_Menace_Recompiled \
  --display-name "Star Wars Episode I: The Phantom Menace Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --runtime-dir mods \
  --disc-hint "Star Wars - Episode I - The Phantom Menace (Europe).cue" \
